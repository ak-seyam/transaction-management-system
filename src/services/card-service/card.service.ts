...
export class CardService {
	...
	async getCardByCardToken(token: string) {
		const card = await this.findCardByCardToken(token);
		if (card) {
			return card;
		}
	...
	private async findCardByCardToken(cardToken: string) {
	...
	async provisionNewCard(token: string) {
		...
		card.limitAmount = 1_000_000;
		card.limitCurrency = "AED";
		card.limitFractionalDigits = 2;
